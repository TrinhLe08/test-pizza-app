...
const UpdateCrustsPage = () => {
  ...
  const [informationUser, setInformationCrusts] = useState({})
  ...
  const handleChangeUpdate = async () => {
    ...
    try {
      const update = await CrustsApi.updateCrusts(dataToUpdate)
      console.log(update);
      setInformationCrusts(update.data)
    } catch (err) {
      console.log(err);
      return
    }
  ...
  return (
    <View style={styles.container}>
       ...
       <Text style={{fontSize: 25}}>Update Crusts:</Text>
...
export default UpdateCrustsPage